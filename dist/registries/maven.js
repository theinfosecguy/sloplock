...
export class MavenCentralRegistryClient {
    timeoutMs;
    retries;
    userAgent;
    fetchImpl;
    ...
    async getPackageUncached(name) {
        ...
        if (coordinate === undefined) {
            return failure(name, "unsupported", "Maven package name must be groupId:artifactId.", false);
        }
        let lastFailure;
    ...
    async fetchPage(name, coordinate, start) {
        ...
        try {
            ...
            if (response.status === 429) {
                return {
                    status: "failure",
                    failure: failure(name, "rate_limited", "Maven Central registry rate limit exceeded.", true)
                };
            }
            if (response.status >= 500) {
                return {
                    status: "failure",
                    failure: failure(name, "server_error", `Maven Central registry returned HTTP ${response.status}.`, true)
                };
            }
            if (!response.ok) {
                return {
                    status: "failure",
                    failure: failure(name, "network_error", `Maven Central registry returned HTTP ${response.status}.`, false)
                };
            }
            let metadata;
            try {
                ...
                return {
                    status: "failure",
                    failure: failure(name, "invalid_response", `Maven Central registry returned invalid JSON: ${message}`, false)
                };
            }
            if (!isMavenCentralSearchResponse(metadata)) {
                return {
                    status: "failure",
                    failure: failure(name, "invalid_response", "Maven Central registry returned invalid package metadata.", false)
                };
            }
...
function parseDocs(name, coordinate, docs) {
    const exactDocs = docs.filter((doc) => doc.g === coordinate.groupId && doc.a === coordinate.artifactId);
...
function searchUrl(coordinate, start) {
    ...
    url.searchParams.set("q", `g:${coordinate.groupId} AND a:${coordinate.artifactId}`);
...
function registryPackageUrl(input) {
    return `https://central.sonatype.com/artifact/${encodeURIComponent(input.groupId)}/${encodeURIComponent(input.artifactId)}`;
}
...
function isMavenCentralSearchResponse(input) {
    ...
    return (typeof metadata.numFound === "number" &&
