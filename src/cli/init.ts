...
export async function runInit(rootDir: string): Promise<InitOutcome> {
...
export function renderInitOutcome(outcome: InitOutcome): string {
  ...
  lines.push(
    "2. Narrow the ecosystems list in sloplock.yml to what this repository uses."
  );
  lines.push(
    "3. Open a pull request that changes dependencies to see SlopLock review it."
  );

  return `${lines.join("\n")}\n`;
}

async function resolveTargetDir(rootDir: string): Promise<string> {
  ...
  throw new UsageError(
    `Target path '${rootDir}' does not exist or is not a readable directory.`
  );
}
...
async function detectTopLevelManifests(targetDir: string): Promise<string[]> {
