...
export function renderCheckText(result: CheckPackagesResult): string {
  const lines: string[] = [];
  ...
  if (result.warnings.length > 0) {
    ...
    for (const warning of result.warnings) {
      lines.push(
        `WARNING ${warning.file === undefined ? warning.message : `${warning.file}: ${warning.message}`}`
      );
    }
    lines.push("");
  }
  ...
  for (const finding of result.findings) {
    lines.push(
      "",
      `${finding.severity.toUpperCase()} ${finding.ecosystem} ${finding.package}`,
      `  Rule: ${finding.rule}`,
...
export function renderCheckJson(result: CheckPackagesResult): string {
...
function describeResult(entry: RegistryResult): string {
...
function toJsonResult(entry: RegistryResult): unknown {
  switch (entry.status) {
    case "found":
      return {
...
function toJsonFinding(finding: PackageCheckFinding): unknown {
  return {
    rule: finding.rule,
